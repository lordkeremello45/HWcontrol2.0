...
sha_file="$TMP_DIR/$(basename "$sha_url")"; curl -fL -o "$sha_file" "$sha_url"
...
case "$family" in
 debian) [ -n "$deb_url" ] && { f="$TMP_DIR/$(asset_name '.deb')"; curl -fL -o "$f" "$deb_url"; install_deb "$f"; echo "HWControl $tag installed via .deb."; exit 0; };;
 rpm) [ -n "$rpm_url" ] && { f="$TMP_DIR/$(asset_name '.rpm')"; curl -fL -o "$f" "$rpm_url"; install_rpm "$f"; echo "HWControl $tag installed via RPM."; exit 0; };;
 arch) [ -n "$arch_url" ] && { f="$TMP_DIR/$(asset_name '.pkg.tar.zst')"; curl -fL -o "$f" "$arch_url"; install_arch "$f"; echo "HWControl $tag installed via Arch package."; exit 0; };;
esac

...
install_archive(){ local file="$1" format="$2" stage="$TMP_DIR/stage"; rm -rf "$stage"; mkdir -p "$stage"; verify_sha "$file"; case "$format" in zst) command -v zstd >/dev/null || { echo 'zstd is required.' >&2; exit 1; }; tar --use-compress-program=zstd -xf "$file" -C "$stage";; gz) tar -xzf "$file" -C "$stage";; esac; source_dir="$(find "$stage" -mindepth 1 -maxdepth 1 -type d|head -n1)"; [ -n "$source_dir" ] || exit 1; $SUDO rm -rf "$INSTALL_DIR"; $SUDO mkdir -p "$INSTALL_DIR"; $SUDO cp -a "$source_dir/." "$INSTALL_DIR/"; $SUDO chmod 0755 "$INSTALL_DIR/bridge-service" "$INSTALL_DIR/ai_engine" 2>/dev/null || true; if [ -f "$INSTALL_DIR/deploy/hwcontrol-bridge.service" ]; then $SUDO install -m0644 "$INSTALL_DIR/deploy/hwcontrol-bridge.service" /etc/systemd/system/hwcontrol-bridge.service; $SUDO systemctl daemon-reload; $SUDO systemctl enable hwcontrol-bridge.service >/dev/null; fi; if [ -d "$INSTALL_DIR/dashboard" ]; then dashboard_bin="$(find "$INSTALL_DIR/dashboard" -type f -name hwcontrol_dashboard -executable|head -n1)"; [ -n "$dashboard_bin" ] && $SUDO ln -sf "$dashboard_bin" /usr/local/bin/hwcontrol; fi; }
case "$family" in
 debian) [ -n "$deb_url" ] && { f="$TMP_DIR/HWControl-$tag.deb"; curl -fL -o "$f" "$deb_url"; install_deb "$f"; echo "HWControl $tag installed via .deb."; exit 0; };;
 rpm) [ -n "$rpm_url" ] && { f="$TMP_DIR/HWControl-$tag.rpm"; curl -fL -o "$f" "$rpm_url"; install_rpm "$f"; echo "HWControl $tag installed via RPM."; exit 0; };;
 arch) [ -n "$arch_url" ] && { f="$TMP_DIR/HWControl-$tag.pkg.tar.zst"; curl -fL -o "$f" "$arch_url"; install_arch "$f"; echo "HWControl $tag installed via Arch package."; exit 0; };;
esac
if [ -n "$zst_url" ]; then f="$TMP_DIR/HWControl-$tag.tar.zst"; curl -fL -o "$f" "$zst_url"; install_archive "$f" zst; echo "HWControl $tag installed via .tar.zst."; exit 0; fi
if [ -n "$gz_url" ]; then f="$TMP_DIR/HWControl-$tag.tar.gz"; curl -fL -o "$f" "$gz_url"; install_archive "$f" gz; echo "HWControl $tag installed via .tar.gz."; exit 0; fi

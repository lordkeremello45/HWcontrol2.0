...
$release = $releases | Where-Object { $_.tag_name -match '-windows$' -and -not $_.draft -and -not $_.prerelease } | Select-Object -First 1
...
if (-not $sha) { throw "The latest Windows release ($($release.tag_name)) has no SHA-256 manifest. Installation is blocked." }

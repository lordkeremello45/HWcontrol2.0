...
HWControl releases use multiple independent verification layers. Use more than one layer when possible. The active CI currently publishes platform-specific SHA-256 manifests and GitHub artifact attestations. The release-wide SHA-512/SHA3-512/GPG/Ed25519 procedures below describe verification formats supported by the project when those assets are present; they are not currently generated by a dedicated release-integrity workflow.
...
A release-wide `SHA512SUMS.txt` manifest can be verified when present; it is not currently guaranteed by the active platform release workflows. SHA-512 is an independent digest algorithm and provides an additional integrity check.
...
A release-wide `SHA3-512SUMS.txt` manifest can be verified when present; it is not currently guaranteed by the active platform release workflows. SHA3-512 is based on the SHA-3/Keccak sponge construction and gives a structurally different integrity algorithm from SHA-256/SHA-512.
...
The private GPG key is stored only as the GitHub Actions repository secret `HWCONTROL_GPG_PRIVATE_KEY_B64`; its passphrase is stored separately as `HWCONTROL_GPG_PASSPHRASE`. When the signing workflow is configured and executed, it imports the key, verifies the expected fingerprint, signs `SHA256SUMS.txt`, verifies the detached signature, and publishes only the public key and signature. No active release workflow currently guarantees creation of these release-wide GPG assets. GitHub recommends keeping sensitive values in Actions secrets and avoiding passing secrets on command lines.
...
When a release-signing workflow is configured with `ED25519_PRIVATE_KEY_B64`, it can sign the release-wide `SHA256SUMS.txt` with Ed25519 and publish:

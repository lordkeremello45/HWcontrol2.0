...
func collectMetrics() HardwareMetrics {
	...
	mergePlatformMetrics(&metrics);mergeDriverInfo(&metrics);if _,err:=load.Avg();err==nil{};metrics.Platform=runtime.GOOS;return metrics
}
...
func loadOrCreateSecret() (string,error) { if secret:=strings.TrimSpace(os.Getenv("HWCONTROL_KEY"));secret!=""&&secret!="replace-me"{return secret,nil};path:=defaultKeyFile();if data,err:=os.ReadFile(path);err==nil{if secret:=strings.TrimSpace(string(data));secret!=""{return secret,nil}};key:=make([]byte,32);if _,err:=rand.Read(key);err!=nil{return "",fmt.Errorf("generate bridge key: %w",err)};secret:=hex.EncodeToString(key);if err:=os.MkdirAll(filepath.Dir(path),0750);err!=nil{return "",fmt.Errorf("create key directory: %w",err)};tmp:=path+".tmp";if err:=os.WriteFile(tmp,[]byte(secret+"\n"),0600);err!=nil{return "",fmt.Errorf("write bridge key: %w",err)};if err:=os.Rename(tmp,path);err!=nil{_=os.Remove(tmp);return "",fmt.Errorf("commit bridge key: %w",err)};return secret,nil }

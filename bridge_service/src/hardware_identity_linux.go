...
func collectHardwareIdentity() HardwareIdentity {
	...
	for _, card := range cards {
		...
		switch vendor {
		case "0x10de": id.GPUVendor = "NVIDIA"
		case "0x1002": id.GPUVendor = "AMD"
		case "0x8086": id.GPUVendor = "Intel"
		default: if vendor != "" { id.GPUVendor = vendor }
		}
		if link, err := filepath.EvalSymlinks(filepath.Join(device, "driver")); err == nil { id.GPUDriver = filepath.Base(link) }
		break
	}
	if output, err := exec.Command("lspci", "-nn", "-d", "::0300").Output(); err == nil {
		line := strings.TrimSpace(strings.SplitN(string(output), "
", 2)[0])
		if idx := strings.Index(line, ": "); idx >= 0 { id.GPUModel = strings.TrimSpace(line[idx+2:]) } else { id.GPUModel = line }
	}
	if id.SystemModel != "" || id.MotherboardModel != "" || id.CPUModel != "" || id.GPUVendor != "" { id.DetectionStatus = "ok" }
	return id
}
